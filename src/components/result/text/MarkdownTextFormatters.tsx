...
export const MarkdownHeading = ({ level, text, lineIndex }: { level: number; text: string; lineIndex: number }) => {
  if (level === 1) {
    return <h2 key={lineIndex} className="text-xl font-bold text-navy mt-6 mb-4">{text}</h2>;
  } else if (level === 2) {
    return <h3 key={lineIndex} className="text-lg font-semibold text-navy mt-5 mb-3">{text}</h3>;
  } else if (level === 3) {
    return <h4 key={lineIndex} className="text-base font-medium text-navy mt-4 mb-2">{text}</h4>;
  } else {
    return <h5 key={lineIndex} className="text-sm font-medium text-navy mt-3 mb-2">{text}</h5>;
  }
};
...
export const PlainText = ({ content, lineIndex }: { content: string; lineIndex: number }) => (
  <p key={lineIndex} className="my-1 text-navy/90">{content}</p>
);

// Component for options header
export const OptionsHeader = ({ lineIndex }: { lineIndex: number }) => (
  <div key={lineIndex} className="text-sm font-medium mt-2 mb-1 text-navy/70">Options:</div>
);
